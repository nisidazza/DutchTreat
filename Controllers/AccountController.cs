        ...
        public async Task<IActionResult> CreateToken([FromBody] LoginViewModel model)
        {
            if (ModelState.IsValid)
            {
                ...
                if (user != null)
                {
                    ...
                    if (result.Succeeded)
                    {
                        //what we need to create the token
                        var claims = new[]
                        {
                            new Claim(JwtRegisteredClaimNames.Sub, user.Email),//subject
                            new Claim(JwtRegisteredClaimNames.Jti, Guid.NewGuid().ToString()),//unique string representative of each token
                            new Claim(JwtRegisteredClaimNames.UniqueName, user.UserName)
                        };

                        //secret to encrypt the token
                        //need to add Token:Key to config.json
